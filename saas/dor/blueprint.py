...
def get_content(obj_id):
    ...
    try:
        ...
        return send_from_directory(node.datastore_path, "{}.content".format(c_hash), as_attachment=True)
...
#         'type': 'object',
#         'properties': {
#             'type': {'enum': ['data_object', 'processor']},
#             'owner_public_key': {'type': 'string'}
#         },
#         'if': {
#             'properties': {'type': {'const': 'data-object'}}
#         },
#         'then': {
#             'properties': {
#                 'descriptor': data_object_descriptor_schema
...
#
#     # add new data object to DOR and obtain the object id
#     status, result = node.dor.add(body['owner_public_key'], body['descriptor'], files['attachment'])
#     return create_signed_response(node, url, status, result)
...
#     owner = node.dor.get_owner(obj_id)
#     if not owner:
#         return create_signed_response(node, url, 404, "Owner for data object '{}' not found.".format(obj_id))
#
...
#
#     # delete data object from DOR
#     status, result = node.dor.add(owner, body['descriptor'], files['attachment'])
#     return create_signed_response(node, url, status, result)
...
# @blueprint.route('/<obj_id>/descriptor', methods=['GET'])
# def get_descriptor(obj_id):
#     url = "GET:/{}/descriptor".format(obj_id)
#
#     # verify the request
#     verified, status_code, message, body, files = verify_request(url, request)
#     if not verified:
